...
if ( ! class_exists( 'BFI_Thumb_1_3' ) ) {
	...
	class BFI_Thumb_1_3 {
		...
		private static function resize_img($img_path, $destfilename, $quality, $params) {
            extract($params);
            ...
            if ( isset( $quality ) ) {
                if ( is_wp_error( $editor->set_quality( $quality )) ) {
                    return false;
                }
            }
        ...
        public static function thumb($url, $params = array(), $single = true) {
            ...
            $destfilename_ret = "{$upload_dir}/{$dst_rel_path}-{$suffix}@2x.{$ext}";
